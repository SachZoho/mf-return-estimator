...
st.set_page_config(page_title="MF Return Estimator", page_icon="chart_with_upwards_trend", layout="wide", initial_sidebar_state="collapsed")

st.title("MF Return Estimator")
st.caption("Estimate today's mutual fund return from underlying stock holdings")

st.info("How it works: Enter a fund name -> App fetches its holdings -> Resolves each stock to an NSE ticker -> Fetches today's price changes -> Computes the weighted estimated return. This is an estimate based on the latest disclosed holdings (monthly). Actual MF NAV may differ.")
...
if st.session_state.selected_fund:
    ...
    with col1:
        ...
        if nav_val:
            st.metric("Latest NAV", f"Rs.{float(nav_val):.4f}", delta=f"as on {nav_date}")
        else:
    ...
    if st.session_state.holdings_data:
        ...
        if non_equity:
            st.caption(f"{len(equity_holdings)} equity holdings will be used for return estimation. {len(non_equity)} non-equity holdings (debt/cash/repo) are excluded.")
...
with alert_col1:
    alert_email = st.text_input("Your email address", placeholder="you@example.com", key="alert_email_input")
...
with alert_col2:
    ...
    if st.session_state.search_results:
        ...
        fund_choices.append((st.session_state.selected_fund["name"], st.session_state.selected_fund["code"]))

    if fund_choices:
        selected_idx = st.selectbox("Fund to track", range(len(fund_choices)), format_func=lambda i: fund_choices[i][0][:60], key="alert_fund_select")
...
if st.button("Set Up Alert", type="primary", disabled=(not alert_email or not alert_fund_code or not gh_token)):
    with st.spinner("Saving alert to GitHub..."):
        ...
        success, msg = save_alert_github(gh_token, gh_owner, gh_repo, alert_email, alert_fund_code, alert_fund_name)
    if success:
...
if gh_token:
    ...
    try:
        ...
        if existing_alerts:
            ...
            remove_idx = st.selectbox("Select alert to remove", range(len(existing_alerts)), format_func=lambda i: f"{existing_alerts[i].get('email', '')} -> {existing_alerts[i].get('scheme_name', '')[:40]}", key="remove_alert_select")
            if st.button("Remove Selected Alert"):
                from alerts import remove_alert_github
                sel = existing_alerts[remove_idx]
                success, msg = remove_alert_github(gh_token, gh_owner, gh_repo, sel.get("email", ""), sel.get("scheme_code", ""))
                if success:
                    st.success(f"{msg}")
                    st.rerun()
                else:
